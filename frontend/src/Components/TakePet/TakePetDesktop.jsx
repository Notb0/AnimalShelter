...
const PageContent = [
    {id:1, text:<p style={{fontFamily:"SansationR", fontSize:"0.8vw"}}>Первое, что необходимо – это выяснить, все ли ваши домочадцы готовы принять в семью нового члена. Помните, что питомец – не игрушка, или сувенир в подарок. Это новый член семьи, который проживет с вами долгую счастливую жизнь.<br/>
    ...
    {id:2, text:<p style={{fontFamily:"SansationR", fontSize:"0.8vw"}}>Вы заполнили анкету и ожидаете решения администрации приюта. Будьте готовы к тому, что к вам придут сотрудники приюта, чтобы убедиться, что животное будет передано в семью, где его ждут, и будут любить со всей ответственностью.<br/>
    ...
    {id:3, text:<p style={{fontFamily:"SansationR", fontSize:"0.8vw"}}>Ваша анкета одобрена. И вы можете забрать своего избранника. Убедитесь, что вы полностью готовы к тому, что прямо сегодня вашасемья станет больше на одного члена. Все ли готово к этой встрече?<br/>
    ...
    При себе необходимо иметь паспорт и переноску (лучше пластиковуюс надежными защелками)Пожалуйста, учитывайте, что без переноски мы животных не отдаем!</p>, image: THREE},
]
